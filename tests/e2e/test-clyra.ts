...
async function testAllWorkflows() {
  ...
  try {
    ...
    // 6. Final Screenshot
    await page.screenshot({ path: 'qa-screenshots/clyra-test-complete.png', fullPage: true });
    ...
    console.log('📸 Full screenshot saved to qa-screenshots/clyra-test-complete.png');

  } catch (error) {
    console.error('\n❌ Test failed:', error);
    await page.screenshot({ path: 'qa-screenshots/clyra-test-failure.png' });
    throw error;
  } finally {
    await browser.close();
  }
