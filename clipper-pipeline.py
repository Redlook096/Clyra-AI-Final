#!/usr/bin/env python3
"""Ultra-light clipper for low-end machines. Captions + 240p video + Whisper on clip. Under 60s."""
...
def main():
    ...
    try:
        ...
        words = []
        # ── STEP 1: Parallel: smallest progressive video + captions ──
        log("download","running",message="Downloading...")
        def dv():
            global vid_path
            # Get smallest progressive stream
            s = (yt.streams.filter(progressive=True, file_extension="mp4")
                 .order_by("resolution").first())
            if not s: s = yt.streams.filter(progressive=True).first()
            if not s: error("No stream")
            s.download(td, filename="video.mp4")
        results = {"video": os.path.join(td,"video.mp4")}
        def gc():
            try:
                caps = yt.captions
        ...
        try:
            ...
            tx = " ".join(f"[{w['start']:.0f}]{w['word']}" for w in words[:1500])
            req = urllib.request.Request("http://localhost:3000/api/deepseek/chat",
                data=json.dumps({"model":"deepseek-chat","messages":[
                    {"role":"system","content":"Find best 30-40s viral clip. Reply ONLY JSON."},
                    {"role":"user","content":f"Find best 30-40s {mt} moment:\n{tx}"}],
                "temperature":0.7,"max_tokens":100,"stream":False}).encode(),
            ...
            if m:
                ...
                cs=float(c.get("start",dur*.15)); ce=float(c.get("end",min(cs+40,dur)))
        ...
        # ── STEP 3: -c copy clip (instant) ──
        log("clip","running",message=f"Cutting {cs}-{ce}s...")
        ...
        # ── STEP 4: Whisper on clip for accurate word timing ──
        log("subtitles","running",message="Subtitles...")
        ...
        subprocess.run([FFMPEG,"-y","-i",cp,"-vf",f"ass={ap}","-c:v","libx264","-preset","ultrafast","-crf","26","-c:a","copy",op],check=True,capture_output=True,timeout=60)
